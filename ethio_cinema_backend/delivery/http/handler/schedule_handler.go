...
func (as *ScheduleHandler) GetAllSchedules(w http.ResponseWriter, r *http.Request) {
	schedules, errs := as.scheduleService.Schedules()
...
func (as *ScheduleHandler) GetSchedules(w http.ResponseWriter, r *http.Request){
	...
	if len(r.URL.Query()) == 0{
		...
		cinemaSchedules, errs := as.scheduleService.CinemaSchedulesbyCinema(uint(id))
		if len(errs) > 0{
...
func (as *ScheduleHandler) PostSchedule(w http.ResponseWriter, r *http.Request) {
	...
	log.Println(as.scheduleService.ScheduleExists(sh.CinemaID, sh.MovieID, sh.Day, sh.StartingTime))
	if as.scheduleService.ScheduleExists(sh.CinemaID, sh.MovieID, sh.Day, sh.StartingTime){
