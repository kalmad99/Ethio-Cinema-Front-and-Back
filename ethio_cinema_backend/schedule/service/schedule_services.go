...
func (s *ScheduleService) ScheduleExists(cinemaid uint, movieid uint, date string, startT string) bool {
	exists := s.scheduleRepo.ScheduleExists(cinemaid, movieid, date, startT)
	return exists
}
